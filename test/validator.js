...
contract('LXValidatorManager', function (accounts) {
    ...
    context("in sidechain", async () => {
        ...
        it('should remove from validator list if account has no TIME', async () => {
            let user = accounts[5];
            let owner = accounts[0];

            await TIME.transfer(user, 100, {from: owner});
            await validatorSet.finalizeChange({from:system});
            ...
            await TIME.transfer(owner, 100, {from: user});
            await validatorSet.finalizeChange({from:system});

            assert.isTrue((await TIME.balanceOf(user)).isZero());
            assert.isFalse(await validatorManager.isValidator(user));
            assert.isFalse(includes(await validatorSet.getValidators(), user));
        })
    });
